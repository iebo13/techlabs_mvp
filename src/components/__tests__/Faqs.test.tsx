...
describe('Faqs', () => {
  it('renders FAQ section header correctly', () => {
    ...
    expect(screen.getByText('Get answers to common questions about TechLabs')).toBeInTheDocument()
  })
  ...
  it('shows FAQ answers when accordion is expanded', () => {
    ...
    const firstAccordion = screen.getByRole('button', {
      name: /where does the techlabs take place/i,
    })
    ...
    expect(
      screen.getByText(
        'We run in multiple cities and online. Check the application page for your city.'
      )
    ).toBeInTheDocument()
  })
  ...
  it('renders FAQ accordion with expandable panels', () => {
    ...
    expect(
      screen.getByText('No. It is designed to fit alongside work or studies (part-time).')
    ).toBeInTheDocument()
