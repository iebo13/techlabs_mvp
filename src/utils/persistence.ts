...
export const loadTrackSelection = (): TrackKey[] => {
    try {
        ...
        if (Array.isArray(parsed)) {
            ...
            return parsed.filter((id): id is TrackKey =>
                typeof id === 'string' && validTrackKeys.includes(id as TrackKey)
            )
        }
...
export const queryParamToTrackIds = (param: string | null): TrackKey[] => {
    ...
    return ids.filter((id): id is TrackKey =>
        validTrackKeys.includes(id as TrackKey)
    )
}
